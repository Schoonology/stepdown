...
function createResultSet(callback, length) {
    return ResultSetMixin.call(length ? new Array(length) : [], callback);
}
...
function _initAsyncProps() {
    ...
    self._initAsyncProp('first', function first() {
        return this.results.alloc('first');
    });
    self._initAsyncProp('collapse', function collapse() {
        return this.results.alloc('collapse');
    });
    self._initAsyncProp('spread', function spread() {
        return this.results.alloc('spread');
    });
}
