...
function ResultSetMixin(callback) {
    ...
    this.callback = function (err, set) {
        this.finished = true;
        if (callback) {
            callback(err, set);
        }
    ...
    this.alloc = ResultSetMixin.alloc;
    this.check = ResultSetMixin.check;
    this.finalize = ResultSetMixin.finalize;
...
function _finish(err /*, ... rest */) {
    if (arguments.length > 0) {
        this._push('spread').apply(this, arguments);
    }
